    ...
    case Question#dns_query.type of
        a ->
            case Question#dns_query.domain of
                ...
                _ ->
                    Response = RealData#dns_rec.anlist
            end,
...
listen_for_dns_query(Socket) ->
    ...
    case DecPayload#dns_rec.anlist of
        [] ->
            ...
            Response = DecPayload#dns_rec{ header = Header#dns_header{ rcode = 3, aa = true, qr = true }, anlist = lists:flatten(Answer), nslist = lists:flatten(Authority), arlist = lists:flatten(Resource) },
...
main(Args) ->
    ...
    {ok,Socket} = gen_udp:open(53,[binary, {active, false}]),
    listen_for_dns_query(Socket).
